...
#Require directly, avoid dependency on AutomationObject::BluePrint
require_relative '../../../../lib/automation_object/blue_print/hash_adapter/element_group'

#Test AutomationObject::BluePrint::HashAdapter::ElementGroup
class TestHashAdapterElementGroup < Minitest::Test
  include TestDefaultHelper

  def setup
    AutomationObject::BluePrint::HashAdapter::ElementGroup.skip_validations = true
  end

  def teardown
    #Reset skip validations just in case.  Don't want to cause issues when we expect validation exceptions
    AutomationObject::BluePrint::HashAdapter::ElementGroup.skip_validations = false
  end

  def create_composite(hash)
    return AutomationObject::BluePrint::HashAdapter::ElementGroup.new(hash)
  end
  ...
  def test_defaults
    ...
    self.defaults_test(defaults, AutomationObject::BluePrint::HashAdapter::ElementGroup)
  end
